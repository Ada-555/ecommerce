...
def generate_invoice_pdf(order):
    """
    Generate an invoice PDF for the given order and save it to media/invoices/.

    Args:
        order: Order model instance

    Returns:
        str: Relative path to the saved PDF file (e.g., 'invoices/inv-{order_number}.pdf')
    """
    # Ensure invoices directory exists
    invoices_dir = os.path.join(settings.MEDIA_ROOT, 'invoices')
    os.makedirs(invoices_dir, exist_ok=True)

    filename = os.path.join(invoices_dir, f"inv-{order.order_number}.pdf")
